...
public class RacingGame extends JPanel implements ActionListener {
	...
	private AnotherCar[] zuti;

	private ObjectsOnRoad[] object;
	...
	public RacingGame(int roadWidth, int roadHeight, int speed) {
		...
		AnimatedVehicle otherVehicle2 = new AnimatedVehicle(minX, maxX, roadHeight, speed + 25);

		//MOJE AUTO
		AnotherCar zuti2 = new AnotherCar(minX, maxX, roadHeight, speed);
		//MOJA MRLJA NA PUTU
		ObjectsOnRoad object2 = new ObjectsOnRoad(minX, maxX, roadHeight, speed);
		
		gameArtifacts = new GameArtifact[] { myVehicle, road, otherVehicle, otherVehicle2, zuti2, object2 };
		animatedGameArtifacts = new AnimatedGameArtifact[] { road, otherVehicle, otherVehicle2, zuti2, object2};
		animatedVehicles = new AnimatedVehicle[] {otherVehicle, otherVehicle2};
		zuti = new AnotherCar[] {zuti2};
		object = new ObjectsOnRoad[] {object2};
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (AnotherCar z : zuti) { // provjera sudara sa zutim autom
				if (myVehicle.colade(z)) {
					myVehicle.crash();
					z.resetPosition();
				}
		}
		
		for (ObjectsOnRoad m : object) { // provjera sudara sa zutim autom
			if (myVehicle.colade(m)) {
				myVehicle.crashOBJ();
				m.resetPosition();
			}
	}
